...
OR (type_of_water_source = 'shared_taps' AND (time_in_queue >= 30))
);
...
SELECT 
    COUNT(CASE
        WHEN results = 'Contaminated: Biological' THEN 1
        ELSE NULL
    END) AS UV_Filter_Count
FROM
    water_source
        LEFT JOIN
    well_pollution ON water_source.source_id = well_pollution.source_id
...
WHERE
 ...
 -- Which towns should we upgrade shared taps first     
    ...
    water_source.type_of_water_source,
    visits.time_in_queue,
    CONCAT('Install ', FLOOR(visits.time_in_queue/30), ' tap(s) nearby') AS Improvement
FROM
    water_source
...
SELECT 
    location.town_name,
    location.province_name,
    AVG(visits.time_in_queue) AS avg_queue_time,
    CONCAT('Install ', FLOOR(AVG(visits.time_in_queue)/30), ' tap(s) nearby') AS Improvement
FROM
    water_source
        INNER JOIN visits ON water_source.source_id = visits.source_id
        INNER JOIN location ON location.location_id = visits.location_id
